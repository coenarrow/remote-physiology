...
- **Entry point**: `neckflix_main.py` (the legacy `main.py` still runs the old
  pipeline but is scheduled for deletion — see Legacy section).
- **Run with**: `uv run python neckflix_main.py --config_file configs/neckflix/<CONFIG>.yaml`
...
- `dataset/data_loader/` — `BaseZarrDataset` / `NeckflixDataset` (lazy, over
  the external zarr cache), `neckflix_config.py` (typed config pattern)
...
**Legacy (works today, dies in Phases 2–6 of the roadmap — do not extend):**

- `main.py`, `config.py`'s yacs bulk, `dataset/data_loader/BaseLoader.py` and
  the twelve per-dataset loaders (each becomes a markdown cache spec in
  Phase 2, then is deleted)
- `neural_methods/trainer/<Model>Trainer.py` files and `BaseTrainer` (each
  dies as its model migrates onto `MultiSignalTrainer`)
...
`neckflix_main.py` modes: `train_and_test`, `only_test`,
...
contain (Phase 2 establishes these in `dataset/data_loader/` — check there
first when asked to implement a cache for a known dataset).
