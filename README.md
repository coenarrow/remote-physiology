...
This is a fork of [rPPG-Toolbox](https://github.com/ubicomplab/rPPG-Toolbox),
extended for **cardiovascular pressure waveform estimation** (ABP, CVP) on the
multimodal Neckflix dataset. It has intentionally diverged from upstream.

The Neckflix pipeline has its own entry point and its own dataset contract:

| | upstream datasets (PURE, UBFC-rPPG, ...) | Neckflix |
|---|---|---|
| entry point | `main.py` | `neckflix_main.py` |
| cache | `.npy`, produced by `DO_PREPROCESS` | zarr, produced by an external preprocessor |
| batch | `(frames, label, filename, chunk_id)` | nested dicts keyed by channel and signal |
| splits | `BEGIN`/`END` percentages | participant filters (LOSO) |
| labels | one per clip | several per clip, with a presence mask |
...
See [`docs/architecture.md`](docs/architecture.md) for the batch-dict contract,
[`CLAUDE.md`](CLAUDE.md) for working conventions, and
[`docs/changelog.md`](docs/changelog.md) for what has diverged. Everything below
this point is the upstream README.

## Environment

`uv sync` is the whole install on Linux, macOS and **Windows** — the upstream
README's advice to use WSL no longer applies to this fork.
...
each one. Linux and macOS are unaffected and keep the published packages.

Into a **fresh** environment, install the build dependencies first, exactly as
`setup.sh` has always done — `mamba-ssm` and `causal-conv1d` build against the
project's own torch rather than an isolated one, so torch has to be there before
they are built:
...
Without `mamba-ssm` (no CUDA, or an unsupported platform), PhysMamba still runs:
`mamba_compat.MambaRef` is a pure-PyTorch selective-SSM block that trains and is
parameter-compatible with the real one. It is a correctness path, not a speed
one.
